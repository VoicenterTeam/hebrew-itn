...
const COMPOUND_PATTERNS = [
  ...
  // Regular number patterns 
  {
    ...
    replacer: (match, number) => {
      return NUMBER_MAPPING[number] || match;
    }
  }
];
...
function normalizeWithPatterns(text) {
  ...
  normalized = normalized.replace(/(רציף|ברציף)\s+(שמונה|שבע|שש|חמש|ארבע|שלוש|שתיים|אחת|תשע|עשר)/g, 
    (match, prefix, number) => {
      const numMap = {
  ...
  normalized = normalized.replace(/\b(אלף|אלפיים|שלושת אלפים|ארבעת אלפים|חמשת אלפים|ששת אלפים|שבעת אלפים|שמונת אלפים|תשעת אלפים|עשרת אלפים|מאה|מאתיים|שלוש מאות|ארבע מאות|חמש מאות|שש מאות|שבע מאות|שמונה מאות|תשע מאות|אחד עשר|שנים עשר|שלושה עשר|ארבעה עשר|חמישה עשר|שישה עשר|שבעה עשר|שמונה עשר|תשעה עשר|עשרים|שלושים|ארבעים|חמישים|שישים|שבעים|שמונים|תשעים|אחד|שניים|שלושה|ארבעה|חמישה|שישה|שבעה|שמונה|תשעה|עשרה|אחת|שתיים|שלוש|ארבע|חמש|שש|שבע|תשע|עשר|שני|שתי|שלושת|ארבעת|חמשת|ששת|שבעת|שמונת|תשעת|עשרת)\b\s+(\w+)/g, 
    (match, number, noun) => {
