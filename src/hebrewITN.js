...
const HUNDREDS_MAP = {
  ...
  'חמש מאות': '500', 'שש מאות': '600',
  'שבע מאות': '700', 'שמונה מאות': '800',
  'תשע מאות': '900'
};
  ...
  const patterns = [
    ...
    { 
      ...
      replacer: (match, tens, units) => {
        const tensVal = TENS_MAP[tens] ? parseInt(TENS_MAP[tens]) : 0;
        const unitsVal = DIGIT_MAP[units] ? parseInt(DIGIT_MAP[units]) : 0;
    ...
    // Hundreds with optional tens and units
    {
      ...
      replacer: (match, hundreds, tens, units) => {
        let value = HUNDREDS_MAP[hundreds] ? parseInt(HUNDREDS_MAP[hundreds]) : 0;
        if (tens) value += TENS_MAP[tens] ? parseInt(TENS_MAP[tens]) : 0;
        if (units) value += DIGIT_MAP[units] ? parseInt(DIGIT_MAP[units]) : 0;
        return `${value}`;
      }
    },
